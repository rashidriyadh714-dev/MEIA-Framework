...
Module: Qualitative Error Extractor


This script traverses the evaluation subset to isolate and extract spatial 
tensors representing False Negatives and Misclassifications across the 
tri-task taxonomy. Outputs inform the manuscript's limitation analysis.
"""
...
logger = logging.getLogger("ErrorExtractor")
...
def save_error_images() -> None:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info(f"Initializing Qualitative Error Extractor on: {device}")
    ...
    logger.info("Commencing misclassification extraction protocol...")
            ...
            for i, (true_val, pred_val) in enumerate(zip(int_lbls, int_preds)):
                if true_val == 1 and pred_val == 0 and i < len(INTENTIONS):
                    ...
                    if saved_counts["Intentions"][name] < MAX_SAVES:
                        filename = output_dir / "Intentions" / f"FalseNegative_{sanitize(name)}_{batch_idx}.png"
                        torchvision.utils.save_image(images[0], filename, normalize=True)
            ...
            for i, (true_val, pred_val) in enumerate(zip(act_lbls, act_preds)):
                if true_val == 1 and pred_val == 0 and i < len(ACTIONS):
                    ...
                    if saved_counts["Actions"][name] < MAX_SAVES:
                        filename = output_dir / "Actions" / f"FalseNegative_{sanitize(name)}_{batch_idx}.png"
                        torchvision.utils.save_image(images[0], filename, normalize=True)
    ...
    logger.info(f"Extraction execution finalized. Isolated {total_saved} erroneous spatial tensors.")
